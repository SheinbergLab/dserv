/*
 * box_persist.h -- portable serialize/validate for box_config_t.
 *
 * Pure C (stdint/string), no hardware. The STORAGE is platform-specific:
 *   - device: RP2350 flash (pico/box_flash.h, flash_range_program)
 *   - sim:    a file (sim/box_sim.c)
 * This module only turns a box_config_t into a versioned, CRC-checked blob and
 * back, so the save/load logic is identical and testable on the Mac.
 *
 * Blob layout:  [magic u32][version u16][len u16][ box_config_t bytes ][crc32 u32]
 ...
 */
#ifndef BOX_PERSIST_H
#define BOX_PERSIST_H
...
#define BOX_PERSIST_VERSION  21  /* v21: static net gateway + subnet mask (net_gw/net_sn).
...
#define BOX_PERSIST_BLOB_MAX  (12 + sizeof(box_config_t))  /* hdr+struct+crc */

static inline uint32_t box_crc32(const uint8_t *p, uint32_t n)
{
...
static inline uint32_t box_persist_serialize(const box_config_t *cfg,
                                              uint8_t *buf, uint32_t bufsz)
{
    ...
    uint8_t *p = buf;
    uint32_t magic = BOX_PERSIST_MAGIC; memcpy(p, &magic, 4); p += 4;
    uint16_t ver = BOX_PERSIST_VERSION; memcpy(p, &ver, 2);   p += 2;
    uint16_t len = (uint16_t) clen;      memcpy(p, &len, 2);   p += 2;
    memcpy(p, cfg, clen);                                      p += clen;
    uint32_t crc = box_crc32(buf, (uint32_t)(p - buf));       memcpy(p, &crc, 4);
    return total;
}
...
static inline int box_persist_deserialize(const uint8_t *buf, uint32_t len,
                                           box_config_t *cfg)
{
    uint32_t clen = (uint32_t) sizeof *cfg;
    ...
    if (magic != BOX_PERSIST_MAGIC) return -1;
    if (ver > BOX_PERSIST_VERSION) return -1;           /* newer format we can't read */
    ...
    if (box_crc32(buf, 8 + blen) != want) return -1;

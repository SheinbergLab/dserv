...
int service_mdns(const char* hostname, const char* service_name, int ds_port, int ess_port) {
  ...
  /* use these as TXT values */
  char ds_port_str[64];
  snprintf(ds_port_str, sizeof(ds_port_str), "%d", ds_port);

  char ess_port_str[64];
  snprintf(ess_port_str, sizeof(ess_port_str), "%d", ess_port);
  ...
  service.txt_record[0] = (mdns_record_t){.name = service.service_instance,
					  ...
					  .data.txt.key = {MDNS_STRING_CONST("dsport")},
					  .data.txt.value = {MDNS_STRING_CONST(ds_port_str)},
					  .rclass = 0,
					  .ttl = 0};
  service.txt_record[1] = (mdns_record_t){.name = service.service_instance,
					  .type = MDNS_RECORDTYPE_TXT,
					  .data.txt.key = {MDNS_STRING_CONST("essport")},

 ...
 */
void Dataserver::set_no_retrigger(ds_datapoint_t *dp) {
  add_datapoint_to_table(dp->varname, dp);

  auto newdp = trigger(dp);

  /* this shouldn't happen, but just in case we don't want to leak memory */
  if (newdp) {
    dpoint_free(newdp);
  }
...
void Dataserver::set(ds_datapoint_t *dpoint)
{
  ...
  if (newdp) {
    /* put this new datapoint (already copied) in the dataserver   */
    /* it can also cause a trigger event, but process_data should  */
    /* not return a new datapoint, as we don't handle that for now */

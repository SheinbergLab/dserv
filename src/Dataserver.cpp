...
Dataserver::Dataserver(int argc, char **argv, int port)
{
  ...
  trigger_thread = std::thread(&Dataserver::process_trigger_requests, this);
				       ...
				       ds_datapoint_t *dpoint)
{
  ds_datapoint_t *old;
  int rval;
  datapoint_table.replace(varname, dpoint);
  return rval;
}
...
int Dataserver::update_datapoint(ds_datapoint_t *dpoint)
{
  ds_datapoint_t *old;
  /*
   * not currently use, but should be fixed to be thread safe
   */
...
/*
 * get datapoint (returns a copy)
 */
ds_datapoint_t *Dataserver::get_datapoint(char *varname)
{
  return datapoint_table.get_dpoint(varname);
}
  
int Dataserver::delete_datapoint(char *varname)
{
  return datapoint_table.delete_dpoint(varname);
}


void Dataserver::do_set(ds_datapoint_t *dp)
{
  //  printf("SET_DPOINT: %s\n", dp->varname);

  add_datapoint_to_table(dp->varname, dp);
  add_to_trigger_queue(dp);
   ...
   */
  ds_datapoint_t *out, *newdp = nullptr;
  if (process_dpoint(dp, &out) == DPOINT_PROCESS_DSERV) {
    /*
     * currently the new point is not passed to processors
     */
    newdp = dpoint_copy(out);
    add_datapoint_to_table(newdp->varname, newdp);
    // add_to_trigger_queue(newdp);
    add_to_notify_queue(newdp);
    add_to_logger_queue(newdp);
  }
}
...
#else
static Tcl_Obj *dpoint_to_tclobj(ds_datapoint_t *dpoint)
{
  Tcl_Obj *obj = NULL;
  int i, n;
  ...
  case DSERV_FLOAT:
	...
	if (i) s << " ";
      ...
      obj = Tcl_NewStringObj((char *) s.str().c_str(), s.str().size());
    }
    break;
  case DSERV_DOUBLE:
    {
...
int Dataserver::dserv_get_event_command(ClientData data, Tcl_Interp * interp, int objc,
					Tcl_Obj * const objv[])
{
  Dataserver *ds = (Dataserver *) data;
  Tcl_Obj *obj;
  ...
  if (dpoint->data.e.dtype != DSERV_EVT) {
    ...
    Tcl_SetObjResult(interp, Tcl_NewStringObj(evt_namebuf, -1));
  }
  dpoint_free(dpoint);
				 ...
				 Tcl_Obj * const objv[])
{
  ...
  /* grab copy of latest trigger point */
  std::unique_lock<std::mutex> tp_mlock(ds->trigger_point_mutex);    
  if (!ds->last_trigger_point) return TCL_OK;
  Tcl_Obj *obj = dpoint_to_tclobj(ds->last_trigger_point);
  tp_mlock.unlock();

  if (obj) Tcl_SetObjResult(interp, obj);
...
void Dataserver::add_tcl_commands(Tcl_Interp *interp)
{
  ...
  Tcl_CreateObjCommand(interp, "dservGetEvent",
		       dserv_get_event_command, this, NULL);
...
int Dataserver::process_requests(void) {
  ...
  while (!m_bDone) {
    ...
    case REQ_TRIGGER:
      {
	const char *script = req.script.c_str();
	printf("REQ_TRIGGER: %s -> %s\n", req.dpoint->varname, script);
...
#if 0
int Dataserver::trigger(ds_datapoint_t *dpoint)
{
  ...
  if (trigger_matches.is_match(dpoint->varname)) {
    ...
    if (trigger_scripts.find(dpoint->varname, script)) {
      ...
      /* set lock around trigger mutex and wait for signal */
      std::unique_lock<std::mutex> mlock(trigger_mutex);
      queue.push_back(client_request);
      trigger_cond.wait(mlock);
    }
    return 1;
  }
  return 0;
  
  /* return copy of point generated by process_dpoint() or nullptr */
  //  if (dp)
...
int Dataserver::add_to_trigger_queue(ds_datapoint_t *dpoint)
{
  if (trigger_matches.is_match(dpoint->varname)) {
    std::string script;
    if (trigger_scripts.find(dpoint->varname, script)) {
      printf("Adding %s to trigger queue\n", dpoint->varname);
      client_request_t client_request;
      client_request.type = REQ_TRIGGER;
      client_request.script = script;
      client_request.dpoint = new_trigger_point(dpoint);
      trigger_queue.push_back(client_request);
      printf("Added %s to trigger queue\n", dpoint->varname);
    }
    return 1;
  }
  return 0;
...
int Dataserver::process_trigger_requests(void) {
  int retcode;
  client_request_t req;
  ...
  while (!m_bDone) {
    ...
    req = trigger_queue.front();
    trigger_queue.pop_front();

    /* push onto main process queue */
    queue.push_back(req);
  }

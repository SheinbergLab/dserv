...
query_callback(int sock, const struct sockaddr* from, size_t addrlen, mdns_entry_type_t entry,
  ...
  char *colon;
  query_response_t *qresp = (query_response_t *) user_data;
    
  mdns_string_t fromaddrstr = ip_address_to_string(addrbuffer, sizeof(addrbuffer), from, addrlen);

  if (entry == MDNS_ENTRYTYPE_ANSWER) {
    qresp->has_txt = 0;		/* default to not a key,val pair */
    qresp->txt_dict[0] = '\0';
    
    colon = strchr(fromaddrstr.str, ':');
    if (colon) {
      snprintf(qresp->host_address, sizeof(qresp->host_address), "%.*s",
	       (int) (colon-fromaddrstr.str), fromaddrstr.str);
      qresp->host_port = strtoul(colon+1, NULL, 10);
    } else {
      snprintf(qresp->host_address, sizeof(qresp->host_address), "%.*s",
  ...
  if (rtype == MDNS_RECORDTYPE_TXT) {
    qresp->has_txt = 1;
    size_t parsed =
      mdns_record_parse_txt(data, size, record_offset, record_length, txtbuffer,
			    sizeof(txtbuffer) / sizeof(mdns_record_txt_t));
    for (size_t itxt = 0; itxt < parsed; ++itxt) {
      if (txtbuffer[itxt].value.length) {
	/* make sure we don't overflow our txt buffer */
	int start_at = strlen(qresp->txt_dict);
	int left = sizeof(qresp->txt_dict)-start_at;
	if (left < 0) left = 0;
	snprintf(&qresp->txt_dict[start_at],
		 left,
		 "%.*s %.*s %.*s",
		 (int) itxt, " ",

/*
 * pico_ain_group.h -- sampling/packing state machine for MCP3204 ANALOG groups.
 *
 * The analog twin of pico_group.h (DI chord groups). A group is a named set of
 * ADC channels (pico_config_t.ain_group_chans[g]) published together under one
 ...
 */
#ifndef PICO_AIN_GROUP_H
#define PICO_AIN_GROUP_H
...
/* Per-group runtime (one per PICO_NAGROUPS slot; lives on core 0). */
typedef struct {
...
static inline int ain_group_batch_eff(const pico_config_t *c, int g, int nchan)
{
...
static inline int ain_group_feed(ain_group_rt_t *rt, const pico_config_t *c, int g,

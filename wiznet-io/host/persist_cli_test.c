...
int main(void)
{
    ...
    act = pico_cli_exec(&c, "name io1", out, sizeof out);
    printf("    > name io1          : %s", out);
    CHECK(act == CLI_OK && strcmp(c.name, "io1") == 0, "set device name");

    act = pico_cli_exec(&c, "pin 5 mode out", out, sizeof out);
    printf("    > pin 5 mode out    : %s", out);
    CHECK(act == CLI_PIN && c.pin_mode[5] == 1, "pin mode out (-> CLI_PIN)");

    act = pico_cli_exec(&c, "pin 5 pulse 500", out, sizeof out);
    CHECK(act == CLI_OK && c.do_pulse_us[5] == 500, "pin pulse");

    act = pico_cli_exec(&c, "dserv ip 10.0.0.5", out, sizeof out);
    printf("    > dserv ip 10.0.0.5 : %s", out);
    CHECK(act == CLI_OK && c.dserv_ip[0] == 10 && c.dserv_ip[3] == 5, "dserv ip");

    act = pico_cli_exec(&c, "dserv port 4620", out, sizeof out);
    CHECK(act == CLI_OK && c.dserv_port == 4620, "dserv port");

    act = pico_cli_exec(&c, "net ip 192.168.1.50", out, sizeof out);
    CHECK(act == CLI_OK && c.net_ip[0] == 192 && c.net_ip[3] == 50, "net ip");

    act = pico_cli_exec(&c, "pin 99 mode out", out, sizeof out);
    CHECK(act == CLI_ERR, "reject out-of-range pin");

    act = pico_cli_exec(&c, "dserv ip 999.1.1.1", out, sizeof out);
    CHECK(act == CLI_ERR, "reject bad ip");

    CHECK(pico_cli_exec(&c, "save", out, sizeof out) == CLI_SAVE, "save action");
    CHECK(pico_cli_exec(&c, "factory", out, sizeof out) == CLI_FACTORY, "factory action");
    CHECK(pico_cli_exec(&c, "bogus", out, sizeof out) == CLI_ERR, "unknown -> err");

...
updateConfigRunButtons() {
    ...
    showManageSubjectsModal() {
        ...
        const removeSubject = async (name) => {
            if (busy || !name) return;
            const current = (this.state.currentSubject || '').toLowerCase();
            if (name.toLowerCase() === current) {
                this.emit('log', {
                    message: `Cannot remove the currently selected subject "${name}"`,
                    level: 'error'
                });
                return;
            }
            ...
            try {
                await this.sendEssCommandAsync(`remove_subject_from_registry {${name}}`);
        ...
        const addSubject = async () => {
            ...
            if (!/^[a-z0-9][a-z0-9_-]*$/.test(name)) {
                this.emit('log', {
                    message: 'Subject ID must start with a letter or digit and contain only letters, digits, _ or -',
                    level: 'error'
                });
                inputEl.focus();
                return;
            }
            if ((this.state.subjects || []).includes(name)) {
                inputEl.focus();
                return;
            }
            setBusy(true, `Adding "${name}"…`);
            try {
                await this.sendEssCommandAsync(`add_subject_to_registry {${name}}`);
                inputEl.value = '';

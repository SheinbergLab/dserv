...
# The wall-clock sanity check that used to live here MOVED INTO THE CORE
# (src/dserv.cpp publishClockHealth -> system/clock_skew_us, system/clock_error).
# It was written here because this is where both clocks were already being read,
# but that was convenience, not correctness: dsconf.tcl gates this subprocess on
# /sys/class/ptp/ptp* existing, so the check ran only on PTP-capable hosts --
# exactly backwards, since a host with no PHC usually has no RTC either and is
# the one most likely to boot with a wrong clock. The fact is a property of
# clock_epoch_offset_us(), not of PTP, and in the core it cannot be gated off.

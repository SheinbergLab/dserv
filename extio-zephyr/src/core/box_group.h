/*
 * box_group.h -- chord-settle state machine for DI pin groups.
 *
 * A group is a set of DI pins (box_config_t.group_pins[g]) published as ONE
 ...
 *
 * Rules (per group, driven by already-DEBOUNCED di events -- box_gpio.h's
...
static inline int group_bit(const box_config_t *c, int g, int pin)
{
    uint32_t m = c->group_pins[g];
    if (pin < 0 || pin >= BOX_NPINS || !((m >> pin) & 1u)) return -1;
    int b = 0;
...
static inline void group_reset(group_rt_t *rt, const box_config_t *c, int g,
...
static inline int group_feed(group_rt_t *rt, const box_config_t *c, int g,
...
static inline int group_poll(group_rt_t *rt, const box_config_t *c, int g,

...
/* a full scan: the first four channels carry the test signal, the rest are 0.
 * AIN_MAX_CH is 8 here (the ADC 20 is 8-channel) while every case below uses
 * masks within ch0..3, which is the point -- widening the part must not change
 * the behaviour of a group that does not use the extra channels. */
static void scan_set(int16_t s[AIN_MAX_CH], int c0, int c1, int c2, int c3)
{ memset(s, 0, sizeof(int16_t) * AIN_MAX_CH);
  s[0]=(int16_t)c0; s[1]=(int16_t)c1; s[2]=(int16_t)c2; s[3]=(int16_t)c3; }

int main(void)
{
    box_config_t cfg; ain_block_t out; int16_t s[AIN_MAX_CH];
